...
public class InventoryHandlerFactoryImpl implements InventoryHandlerFactory {
  ...
  /**
   * Constructor with dependency injection. Spring automatically injects all beans implementing
   * InventoryHandler.
   *
  ...
  public InventoryHandlerFactoryImpl(List<InventoryHandler> handlers) {
    ...
    this.handlerMap =
        handlers.stream().collect(Collectors.toMap(InventoryHandler::getType, Function.identity()));
    log.info(
        "InventoryHandlerFactory initialized with {} handlers: {}",
        handlerMap.size(),
        handlerMap.keySet());
  }
  ...
  public InventoryHandler getHandler(String handlerType) {
    ...
    if (handler == null) {
      ...
      throw new IllegalArgumentException(
          "Unsupported handler type: " + handlerType + ". Available types: " + handlerMap.keySet());
    }
